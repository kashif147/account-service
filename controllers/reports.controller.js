  ...
  const [rows, coa] = await Promise.all([
    GL.aggregate([
      ...
      { $group: {
          ...
          debit: { $sum: { $cond: [{ $eq: ["$entries.dc","D"] }, "$entries.amount", 0] } },
          credit:{ $sum: { $cond: [{ $eq: ["$entries.dc","C"] }, "$entries.amount", 0] } }
      } }
    ]),
    CoA.find({}).lean()
  ]);
  const byCode = Object.fromEntries(coa.map(a => [a.code, a]));
  return mapNames(byCode, rows).sort((a,b)=>a.accountCode.localeCompare(b.accountCode));
}
...
async function incomeStatement(startISO, endISO) {
  ...
  const sum = (arr) => arr.reduce((s,x)=>s + (x.type==="Income" ? -x.net : x.net), 0);
  ...
  return {
    ...
    totals: {
      ...
      profit:       Number((sum(income) - sum(contraIncome) - sum(expenses)).toFixed(2))
    }
  };
}
...
async function membersBalancesAsOf(endISO) {
  ...
  const rows = await GL.aggregate([
    ...
    { $unwind: "$entries" },
    { $match: { "entries.accountCode": { $in: ["1400","2020"] }, "entries.memberId": { $exists: true, $ne: null } } },
    { $group: {
        _id: { memberId: "$entries.memberId", accountCode: "$entries.accountCode" },
        debit:  { $sum: { $cond: [{ $eq: ["$entries.dc","D"] }, "$entries.amount", 0] } },
        credit: { $sum: { $cond: [{ $eq: ["$entries.dc","C"] }, "$entries.amount", 0] } }
    } },
    { $project: {
  ...
  for (const r of rows) {
    ...
    if (r.accountCode === "1400") byMember[r.memberId].ar1400 = Number(r.amount.toFixed(2));
    if (r.accountCode === "2020") byMember[r.memberId].poa2020 = Number(r.amount.toFixed(2));
  }
...
async function clearingReconciliation(startISO, endISO) {
  ...
  const rows = await GL.aggregate([
    ...
    { $project: { accountCode: "$_id", debit: 1, credit: 1, net: { $subtract: ["$debit","$credit"] } } }
  ]);
...
export async function monthEnd(req, res, next) {
  try {
    ...
    const report = lock
      ? await snapshotOrCompute("month-end", label, { startISO, endISO }, compute, req.user?.id, notes)
      : await compute();
...
export async function yearEnd(req, res, next) {
  try {
    ...
    const report = lock
      ? await snapshotOrCompute("year-end", label, { startISO, endISO }, compute, req.user?.id, notes)
      : await compute();
...
async function snapshotOrCompute(type, label, range, computeFn, lockedBy, notes) {
