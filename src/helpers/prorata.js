...
export function diffDaysInclusive(fromISO, toISO) {
  ...
  if (d < 0) throw new Error(`Invalid range ${fromISO} → ${toISO}`);
  return d;
}
...
export function yearBoundsFrom(dateISO) {
  ...
  return { startISO: d.startOf("year").format("YYYY-MM-DD"),
...
export function prorataForPeriod(annualFee, fromISO, toISO) {
  ...
  if (fromY !== toY) throw new Error("Pro-rata period must be within one calendar year");
